...
dev_out = adafruit_dht.DHT22(board.D17)
dev_in  = adafruit_dht.DHT22(board.D4)
...
def read2dev(dev_in, dev_out, timeout=sleep_secs):
	...
	while True:
		...
		if tmp<timeout and error_in == 0:
			...
			if not check_error(temp_in, ctrl="temp"): temp_arr_in.append(temp_in)
			if not check_error(hum_in,  ctrl="hum"): hum_arr_in.append(hum_in)
			correct_in = correct_in+1
		if tmp<timeout and error_out == 0:
			if use_print: print(temp_arr_out, hum_arr_out)
			if not check_error(temp_out, ctrl="temp"): temp_arr_out.append(temp_out)
			if not check_error(hum_out,  ctrl="hum"): hum_arr_out.append(hum_out)
